...
import numpy as np
from Transformer.attention import MultiheadAttention, SelfAttention, SimpleAttention
...
class TestMultiheadAttention(unittest.TestCase):
    def test_call(self):
        ...
        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            q = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, None, hidden_dim])
            k = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, None, hidden_dim])
    ...
    def test_split_head_graph(self):
        ...
        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            model = MultiheadAttention(hidden_dim=hidden_dim, head_num=head_num, dropout_rate=0.1)
            x = tf.compat.v1.placeholder(dtype=tf.float32, shape=[batch_size, None, None])
            y = model._split_head(x)
