...
class FeedForwardNetwork(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_dim: int, dropout_rate: float,
        ...
        self.filter_dense_layer = \
            tf.keras.layers.Dense(hidden_dim * 4, use_bias=True,
...
class ResidualNormalizationWrapper(tf.keras.layers.Layer):
    ...
    def __init__(self, layer: tf.keras.layers.Layer,
                 dropout_rate: float, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.layer = layer
...
class LayerNormalization(tf.keras.layers.Layer):
    ...
    def call(self, x: tf.Tensor, epsilon: float = 1e-6) -> tf.Tensor:
        ...
        variance = tf.reduce_mean(tf.square(x - mean), axis=[-1],
                                  keepdims=True)
