...
class MultiheadAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_dim: int, head_num: int,
                 dropout_rate: float, *args, **kwargs):
        '''
        コンストラクタです。
        ...
        self.q_dense_layer = tf.keras.layers.Dense(hidden_dim,
                                use_bias=False, name='q_dense_layer')
        self.k_dense_layer = tf.keras.layers.Dense(hidden_dim,
                                use_bias=False, name='k_dense_layer')
        self.v_dense_layer = tf.keras.layers.Dense(hidden_dim,
                                use_bias=False, name='v_dense_layer')
        self.output_dense_layer = tf.keras.layers.Dense(hidden_dim,
                                use_bias=False, name='output_dense_layer')
    ...
    def call(
        ...
        # ここで q と k の内積を取ることで、query と
        # key の関連度のようなものを計算します。
        logit = tf.matmul(q, k, transpose_b=True)  # [batch_size, head_num,
                                                   # q_length, k_length]
        # mask は pad 部分などが1, 他は0
    ...
    def _split_head(self, x: tf.Tensor) -> tf.Tensor:
        ...
        with tf.name_scope('split_head'):
            ...
            x = tf.reshape(x, [batch_size, length, self.head_num,
...
class SimpleAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, depth: int, *args, **kwargs):
        ...
        self.q_dense_layer = tf.keras.layers.Dense(depth,
                                use_bias=False, name='q_dense_layer')
        self.k_dense_layer = tf.keras.layers.Dense(depth,
                                use_bias=False, name='k_dense_layer')
        self.v_dense_layer = tf.keras.layers.Dense(depth,
                                use_bias=False, name='v_dense_layer')
        self.output_dense_layer = tf.keras.layers.Dense(depth,
                                use_bias=False, name='output_dense_layer')
    ...
    def call(self, input: tf.Tensor, memory: tf.Tensor) -> tf.Tensor:
        ...
        # ここで q と k の内積を取ることで、query と key
        # の関連度のようなものを計算します。
